...
import 'package:pfe_banking_system/screens/Virement/screens/virement%20ver%20mes%20comptes/TransactioncertifVi.dart';
...
class Codeverifmone extends StatefulWidget {
  String? otpCode;
  String BankId;
  String montant;
  String senderRIB;
  String receiverRIB;
  int? id;
  Codeverifmone({super.key, required this.otpCode, required this.montant, required this.BankId, required this.senderRIB, required this.receiverRIB,  this.id});

  @override
  State<Codeverifmone> createState() => _CodeverifmoneState();
}

class _CodeverifmoneState extends State<Codeverifmone> {
  String? BankOpId;
                ...
                InkWell(
                  onTap: () async{
                    ...
                    if(code == widget.otpCode?.substring(widget.otpCode!.length - 6)){
                      ...
                      Provider.of<TransferOpProvider>(context,listen: false).setNameCreditNum(namecreditnum: "");
                      ...
                      if(TransferOp != null && TransferOp.id != null){
                        setState(() {
                          BankOpId = TransferOp.id.toString();
                        });
                        print("TransferOpId created successfully ${BankOpId}");
                        await Provider.of<TransferOpProvider>(context,listen: false).addTransferOpToBankingAccount(BankOpId!, widget.BankId);
                      } else {
                        print("Error: TransferOp or TransferOp.id is null");
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text("Erreur lors de la création de l'opération de transfert"),
                            backgroundColor: Colors.red,
                          ),
                        );
                        return;
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => TransactionscertifVi(
                            montant: widget.montant,
                            senderRIB: widget.senderRIB,
                            receiverRIB: widget.receiverRIB,
                            date: "${now.day}-${now.month}-${now.year}",
                            bankOpId: BankOpId ?? "N/A",
                            id : widget.id
                          ),
                        ),
                      );
