...
import 'package:pfe_banking_system/screens/statistiques/screens/Epargne.dart';
import 'package:provider/provider.dart';

import '../provider/EpargneProvider.dart';

class addEpargne extends StatefulWidget {
  int? id;
  addEpargne({super.key, required  this.id});

  @override
  State<addEpargne> createState() => _addEpargneState();
}

class _addEpargneState extends State<addEpargne> {
  ...
  Future<void> getExistingBudget()async{
    try{
      final result = await Provider.of<MaxBudgetProvider>(context,listen: false).getMaxBudgetByType("epargne");
      if(result != null){
        setState(() {
          BudgetId = result.id;
        });
              ...
              InkWell(
                onTap: () async{
                  if(key.currentState!.validate()){
                    Provider.of<EpargneProvider>(context,listen: false).setAmount(amount: int.parse(montant.text));
                    Provider.of<EpargneProvider>(context,listen: false).setType();
                    Provider.of<EpargneProvider>(context,listen: false).setName(name: nom.text);
                    Provider.of<EpargneProvider>(context,listen: false).setTime(time: "${formattedTime}");
                    Provider.of<EpargneProvider>(context,listen: false).setDate(date: "${now.day}-${now.month}-${now.year}");
                    await Provider.of<EpargneProvider>(context,listen: false).saveEpargne();
                    final epargne = Provider.of<EpargneProvider>(context,listen: false).epargne;
                    if(epargne != null){
                      setState(() {
                        expenseId = epargne.id.toString();
                      });
                    }
                    await Provider.of<ExpenseProvider>(context,listen: false).addExpenseToBankingAccount(widget.id.toString(), expenseId!);
                    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Epargne(id : widget.id!)));
                  }
                },
                child: Container(
                  height: 50,
