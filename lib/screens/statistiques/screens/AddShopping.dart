...
import 'package:pfe_banking_system/screens/statistiques/screens/Shopping.dart';
import 'package:provider/provider.dart';

class addShopping extends StatefulWidget {
  int? id;
   addShopping({super.key,  this.id});
...
class _addShoppingState extends State<addShopping> {
  ...
  Future<void> getExistingBudget()async{
    try{
      final result = await Provider.of<MaxBudgetProvider>(context,listen: false).getMaxBudgetByType("shopping");
      if(result != null){
        setState(() {
          BudgetId = result.id;
        });
              ...
              InkWell(
                onTap: () async{
                  if(key.currentState!.validate()){
                    Provider.of<ShoppingProvider>(context,listen: false).setAmount(amount: int.parse(montant.text));
                    Provider.of<ShoppingProvider>(context,listen: false).setType();
                    Provider.of<ShoppingProvider>(context,listen: false).setName(name: nom.text);
                    Provider.of<ShoppingProvider>(context,listen: false).setTime(time: "${formattedTime}");
                    Provider.of<ShoppingProvider>(context,listen: false).setDate(date: "${now.day}-${now.month}-${now.year}");
                    await Provider.of<ShoppingProvider>(context,listen: false).saveShopping();
                    final expense = Provider.of<ShoppingProvider>(context,listen: false).shopping;
                    if(expense != null){
                      setState(() {
                    ...
                    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Shopping(id: widget.id!)));
                  }
                },
                child: Container(
                  height: 50,
