...
class addEspeces extends StatefulWidget {
  int? id ;
   addEspeces({super.key, this.id});

  @override
  State<addEspeces> createState() => _addEspecesState();
}
...
class _addEspecesState extends State<addEspeces> {
  ...
  Future<void> getExistingBudget()async{
    try{
      final result = await Provider.of<MaxBudgetProvider>(context,listen: false).getMaxBudgetByType("especes");
      if(result != null){
        setState(() {
          BudgetId = result.id;
        });
              ...
              InkWell(
                onTap: () async{
                  if(key.currentState!.validate()){
                    Provider.of<EspecesProvider>(context,listen: false).setAmount(amount: int.parse(montant.text));
                    Provider.of<EspecesProvider>(context,listen: false).setType();
                    Provider.of<EspecesProvider>(context,listen: false).setName(name: nom.text);
                    Provider.of<EspecesProvider>(context,listen: false).setTime(time: "${formattedTime}");
                    Provider.of<EspecesProvider>(context,listen: false).setDate(date: "${now.day}-${now.month}-${now.year}");
                    await Provider.of<EspecesProvider>(context,listen: false).saveEspeces();
                    final expense = Provider.of<EspecesProvider>(context,listen: false).especes;
                    if(expense != null){
                      setState(() {
                    ...
                    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Especes(id: widget.id!,)));
                  }
                },
                child: Container(
                  height: 50,
