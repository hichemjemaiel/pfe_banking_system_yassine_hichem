...
import 'package:pfe_banking_system/screens/CreationEntreprise/screens/CreationEntrepriseSeven.dart';

import 'package:pfe_banking_system/screens/CreationParticulier/screens/CreationParticulierSeven.dart';
import 'package:provider/provider.dart';

import 'CreationParticulier/model/AgencyModel.dart';
import 'CreationParticulier/provider/AgencyProvider.dart';
...
class AjoutCompte extends StatefulWidget {
  int? id;
  String email;
   AjoutCompte({super.key, required this.id, required this.email});

  @override
  State<AjoutCompte> createState() => _AjoutCompteState();
}
                    ...
                    Padding(padding: EdgeInsets.only(right: 10),child: InkWell(
                      onTap: () async{
                        if(selectedAgencyIndex != -1){
                          ...
                          await Future.wait( [
                            Provider.of<BankingProvider>(context,listen: false).addBankingAccountToClient(bankingAccountId!, widget.id.toString()),
                            Provider.of<BankingProvider>(context,listen: false).addBankingAccountToAccountType(bankingAccountId!, accountTypeId!),

                          ]);
                          Provider.of<BankingProvider>(context,listen: false).addBankingAccouuntToAgency(bankingAccountId!, selectedAgencyIndex.toString());
                          Navigator.push(context, MaterialPageRoute(builder: (context) => Confirmaccount(email : widget.email)));
                        }
