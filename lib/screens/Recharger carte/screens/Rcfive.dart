...
import '../provider/DechargeCardProvider.dart';
import '../provider/RechargeCardProvider.dart';
import 'TransactionVertifCard.dart';

class Rcfive extends StatefulWidget {
  String montant;
  String? otpCode;
  String card;
  int ind3;
  ...
  @override
  State<Rcfive> createState() => _RcfiveState();
}

class _RcfiveState extends State<Rcfive> {
  String? BankOpId;
  String? dechargeId;
  ...
  Future<void> _resendOTP() async {
    ...
    try {
      // Generate new OTP (replace with your phone number)
                    ...
                    InkWell(
                      onTap: _isLoading ? null : () async {
                        ...
                        if (code == widget.otpCode?.substring(widget.otpCode!.length - 6)) {
                          ...
                          try {
                            final dechargeProvider = Provider.of<DechargeCardProvider>(context, listen: false);
                            final DateTime now = DateTime.now();

                            // Set all required fields in the provider
                            dechargeProvider.setCreditRib(creditRib: widget.creditRib);
                            ...
                            dechargeProvider.setBanque(banque: bankName);
                            dechargeProvider.setMontant(montant: int.parse(widget.montant));

                            // Set date and time to avoid null values
                            dechargeProvider.setDate(date: DateFormat('yyyy-MM-dd').format(now));
                            dechargeProvider.setTime(time: DateFormat('HH:mm:ss').format(now));

                            // Save the decharge card
                            await dechargeProvider.saveDechargeCard(widget.card);

                            // Get the created decharge and validate it
                            final decharge = dechargeProvider.dechargeCard;
                            if (decharge == null) {
                              throw Exception("Échec de la création de la décharge");
                            }

                            if (decharge.id == null) {
                              throw Exception("ID de décharge manquant dans la réponse du serveur");
                            }

                            setState(() {
                              dechargeId = decharge.id.toString();
                            });

                            print("Decharge created successfully with ID: $dechargeId");

                            // Add the decharge to the card
                            await dechargeProvider.addDechargeCardToCard(dechargeId!, widget.card);
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => TransactionscertifCard(
                                    ...
                                    creditRib: widget.creditRib,
                                    cardNumber: widget.card,
                                    cardHolderName: null,
                                    banque: bankName,
                                    operationType: "Décharge de carte",
                                    reference: dechargeId,
                                    transactionDate: now,
                                  ),
                                ),
                              );
