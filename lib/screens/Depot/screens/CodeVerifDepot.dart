...
class Codeverifdepot extends StatefulWidget {
  int AgencyId;
  int BankingAccountId;
  String? otpCode;
  Codeverifdepot({super.key, required this.AgencyId, required this.BankingAccountId, required this.otpCode});

  @override
  State<Codeverifdepot> createState() => _CodeverifdepotState();
}

class _CodeverifdepotState extends State<Codeverifdepot> {
  String? BankOpId;
                ...
                InkWell(
                  onTap: () async{
                    ...
                    if(code == widget.otpCode?.substring(widget.otpCode!.length - 6)){
                      ...
                      final BankingOp = Provider.of<DepositOpProvider>(context,listen: false).depositOpModel;
                      if(BankingOp !=null){
                        setState(() {
                          BankOpId = BankingOp.id.toString();
                        });
                        print("BankingOp : ${BankOpId}");
                        print("BankingOp montant: ${BankingOp.montant}");
                        print("BankingOp date : ${BankingOp.date}");
                      }
                      await Provider.of<BankingOpProvider>(context,listen: false).addBankingOperationToAgency(BankOpId!, widget.AgencyId.toString()!);
                      await Provider.of<DepositOpProvider>(context,listen: false).addDepositOpToBankingAccount(BankOpId!, widget.BankingAccountId.toString()!);
